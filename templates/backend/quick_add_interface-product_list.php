...
if ( ! empty( $products ) ) : ?>
	...
	<?php wp_nonce_field( 'wps_save_product_quick_interface' ); ?>
	  <table class="wp-list-table widefat wps-product-mass-interface-table">
		  <thead>
			  <tr>
				  <td class="check-column manage-column column-cb check-column">
				  ...
				  <th><?php _e( 'Picture', 'wpshop' ); ?></th>
					<?php echo $this->display_products_column( 'post_title', __( 'Title' ) ); ?>
				  <th class="description"><?php _e( 'Description', 'wpshop' ); ?></th>
					...
					<?php foreach ( $tab_def as $col ) : ?>
						<?php echo $this->display_products_column( $col['code'], $col['name'] ); ?>
					<?php endforeach; ?>
					<?php endif; ?>
			  </tr>
		  </thead>

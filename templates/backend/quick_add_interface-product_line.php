...
<tr class="<?php echo $class; ?>" id="trPid_<?php echo $product['post_datas']->ID; ?>">
	<th scope="row" class="check-column">
		<input type="checkbox" class="wps-save-product-checkbox" name="wps_product_quick_save[]" value="<?php echo $product['post_datas']->ID; ?>" />
	</th>

	<?php if ( $product['post_datas']->post_type === WPSHOP_NEWTYPE_IDENTIFIER_PRODUCT_VARIATION ) : ?>

	<td class="wps_mass_interface_line" colspan="3">
	...
	<td class="wps_mass_interface_line">
		<a href="#" class="wps_mass_interface_picture_container wps_del_picture_to_product_in_mass_interface" id="wps_del_picture_to_product_in_mass_interface_<?php echo $product['post_datas']->ID; ?>"><?php echo get_the_post_thumbnail( $product['post_datas']->ID, 'thumbnail' ); ?></a>
	...
	<td class="wps_mass_interface_line">
		<textarea rows="1" id="wps_product_description_<?php echo $product['post_datas']->ID; ?>" name="wps_mass_interface[<?php echo $product['post_datas']->ID; ?>][post_content]"><?php echo nl2br( $product['post_datas']->post_content );?></textarea>
	</td>
...
<th scope="row" class="check-column"><input type="hidden" class="is_row" name="concur[<?php echo $product['post_datas']->ID; ?>][is_row][]" value="1"></th>
<td class="wps_mass_interface_line"><input type="text" placeholder="Date" class="datepicker_concur" value="<?php echo $concur['date']; ?>" name="concur[<?php echo $product['post_datas']->ID; ?>][date][]"></td>
<td class="wps_mass_interface_line"><input type="text" placeholder="Lien" name="concur[<?php echo $product['post_datas']->ID; ?>][link][]" value="<?php echo $concur['link']; ?>"></td>
<td class="wps_mass_interface_line">
<?php // End if().
...
// echo preg_replace( '/name=".*?"/i', 'name="concur[' .$product['post_datas']->ID. '][name][]"', $att['field_definition']['output'] ); ?>
	...
	<td class="wps_mass_interface_line"><input type="text" placeholder="Prix" name="concur[<?php echo $product['post_datas']->ID; ?>][price][]" value="<?php echo $concur['price']; ?>"></td>
	<td class="wps_mass_interface_line del_concur" style="color: red; cursor: pointer;">Supprimer</td>
	<?php for ( $j = 0; $j < $i -2;
	$j++ ) {
		echo '<td class="wps_mass_interface_line"></td>';
} ?>
</tr>
<?php
}
	$output_specs = array(
		'page_code' => WPSHOP_NEWTYPE_IDENTIFIER_PRODUCT,
		'element_identifier' => $product['post_datas']->ID,
		'field_id' => WPSHOP_NEWTYPE_IDENTIFIER_PRODUCT . '_' . $product['post_datas']->ID . '_%ID%',

	);
?>
<tr class="<?php echo $class; ?> concurs cloner" style="display:none" data-id="<?php echo $product['post_datas']->ID; ?>">
	<th scope="row" class="check-column"><input type="hidden" class="is_row" name="concur[<?php echo $product['post_datas']->ID; ?>][is_row][]" value="0"></th>
	...
	<td><?php $att = wpshop_attributes::display_attribute( 'concurents_', 'backend', $output_specs );
	echo preg_replace( '/name=".*?"/i', 'name="concur[' . $product['post_datas']->ID . '][name][]"', preg_replace( '/class=".*?"/i', 'class="wpshop_product_attribute_concurents_ chosen_select_concur"', $att['field_definition']['output'] ) ); ?></td>

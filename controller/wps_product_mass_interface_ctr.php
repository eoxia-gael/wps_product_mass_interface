...
class wps_product_mass_interface_ctr {
	 ...
	 * @return string
	 */
	function display_products_list( $default = 1, $page = 0, $order_by = 'ID', $order = 'DESC' ) {
		...
		$products = $wps_product_mass_interface_mdl->get_quick_interface_products( $default, $page, $product_limit, $order_by, $order );
	...
	/**
	 * Display column for WP_List_Table
	 *
	 * @method display_products_column
	 * @param  string $code Column code (see order_by wps_product_mass_interface_mdl::get_quick_interface_products).
	 * @param  string $name Display name for column.
	 * @return string                        Output template column
	 */
	function display_products_column( string $code, string $name ) {
		$output = '';
		ob_start();
		require( wpshop_tools::get_template_part( WPS_PDCT_MASS_DIR, WPS_PDCT_MASS_TEMPLATES_MAIN_DIR, 'backend', 'quick_add_interface', 'product_col' ) );
		$output = ob_get_contents();
		ob_end_clean();
		return $output;
	}
	...
	function wps_display_mass_edit_interface() {
		...
		// Display product tab
		$order_by = ! empty( $_GET['order_by'] ) ? sanitize_title( $_GET['order_by'] ) : 'ID';
		$order = ! empty( $_GET['order'] ) ? sanitize_title( $_GET['order'] ) : 'DESC';
	...
	function get_products_pagination( $current_page, $attribute_set_id = 1 ) {
		...
		$query = $wpdb->prepare( "SELECT COUNT( * ) AS products_number
	...
	function wps_save_product_quick_interface() {
			...
			$i++ ) {
				...
				if ( empty( $concur[ $i ]['is_row'] ) ) {
					...
					if ( ! $sep ) {
						...
						$concur[ $i ]['price'] = floatval( preg_replace( '/[^0-9]/', '', substr( $concur[ $i ]['price'], 0, $sep ) ) . '.' . preg_replace( '/[^0-9]/', '', substr( $concur[ $i ]['price'], $sep + 1, strlen( $concur[ $i ]['price'] ) ) ) );
					}
			...
			foreach ( $wps_product_quick_save as $product_to_save ) {
				...
				if ( ! empty( $updated_post ) ) {
					...
					if ( ! empty( $wps_mass_interface[ $product_to_save ]['picture'] ) ) {
						...
						if ( $wps_mass_interface[ $product_to_save ]['picture'] != 'deleted' ) {
										wp_update_post( array(
											'ID' => (int) $wps_mass_interface[ $product_to_save ]['picture'],
											'post_parent' => $updated_post,
										) );
					...
					if ( empty( $data_to_save['wpshop_product_attribute']['varchar']['barcode'] ) ) {
						...
						$barcode_value = wpshop_attributes::wpshop_att_val_func( array(
							'pid' => $data_to_save['post_ID'],
							'attid' => $attid,
						) );
		...
		if ( $status ) {
			...
			if ( ! empty( $total_updated_products ) ) {
				$response = sprintf( __( 'All selected products do not be updated. %1$d on %2$d products have been successfully updated', 'wpshop' ), $total_updated_products, $count_products_to_update );
			} else {
